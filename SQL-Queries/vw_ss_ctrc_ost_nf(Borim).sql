...
--CREATE VIEW [dbo].[VW_CTRC_OST_NF_SS] AS 
...
C.SITUAC AS SITUAC, C.OUTROS AS OUTROS, C.ICMVLR AS ICMVLR, C.CODDES, MDES.ESTADO AS UF_DESTINO, C.CODREM, MREM.ESTADO AS UF_ORIGEM, C.CODPAG,
C.TIPCON AS TIPO_DOC, NULL AS CODNOT, NULL AS FILNOT, NULL AS SERNOT, NULL AS CTRC_OST, NULL AS FILCON, C.DATLIQ AS DATLIQ, C.DATPAG AS DATPAG, 
C.TOTFRE AS TOTFRE, C.VLRLIQ AS VLRLIQ, C.FREVAL AS FREVAL, C.SITDUP AS SITDUP, C.VLRPED AS PEDAGIO, 
(SELECT ISNULL(SUM(QUANTI), 0) FROM RODNFC I WHERE I.CODCON = C.CODCON AND I.SERCON = C.SERCON AND I.CODFIL = C.CODFIL) AS SOMA_QUANTI,
...
UNION ALL 
SELECT 'OST' AS TIPO, O.SITUAC AS SITUACAO, O.DATCAD AS DATCAD, O.CODFIL AS CODFIL, NULL AS UFCOLETA, FIL.RAZSOC AS NOM_FILIAL, 
O.CODIGO AS CODCON, O.SERORD AS SERCON, O.PLACA AS PLACA, O.PLACA2 AS PLACA2, O.PLACA3 AS PLACA3, O.FREPES AS FREPES, O.CODMOT AS CODMOT, 
O.SITUAC AS SITUAC, O.OUTROS AS OUTROS, O.ICMVLR AS ICMVLR, O.CODDES, MDES.ESTADO AS UF_DESTINO, O.CODREM, REM.ESTADO AS UF_ORIGEM, O.CODPAG, 
O.TIPORD AS TIPO_DOC, O.CODNOT, O.FILNOT, O.SERNOT, O.CODIGO AS CTRC_OST, O.FILCON, O.DATLIQ AS DATLIQ, O.DATPAG AS DATPAG, O.TOTFRE AS TOTFRE, 
O.VLRLIQ AS VLRLIQ, O.FREVAL AS FREVAL, O.SITDUP AS SITDUP, O.VLRPED AS PEDAGIO, 
...
D.SERSUB AS SERCON, NULL AS PLACA, NULL AS PLACA2, NULL AS PLACA3, NULL AS FREPES, NULL AS CODMOT, D.SITUAC AS SITUAC, NULL AS OUTROS, 
NULL AS ICMVLR, NULL AS CODDES, NULL AS UF_DESTINO, NULL AS CODREM, NULL AS UF_ORIGEM, D.CODCLIFOR AS CODPAG, 'N' AS TIPO_DOC, D.CODIGO AS CODNOT,
D.CODFIL AS FILNOT, D.SERSUB AS SERNOT, NULL AS CTRC_OST, NULL AS FILCON, NULL AS DATLIQ, NULL AS DATPAG, 
(D.VLRSER+D.VLRISS) AS TOTFRE, (D.VLRSER) AS VLRLIQ, NULL AS FREVAL, D.SITDUP AS SITDUP, 0 AS PEDAGIO, 0 AS SOMA_QUANTI,
0 AS SOMA_PESOKG, 0 AS SOMA_VLRMER, '' AS NOTAS 
FROM ESTNOT D INNER JOIN RODFIL FIL ON D.CODFIL = FIL.CODFIL 
